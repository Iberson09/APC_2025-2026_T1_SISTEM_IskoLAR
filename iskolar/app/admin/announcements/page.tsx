...
export default function AnnouncementManagementPage() {
  ...
  const fetchAnnouncements = async () => {
    ...
    try {
      ...
      setNotification({ message: 'Failed to fetch announcements.', type: 'error' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSaveAnnouncement = async (formData: AnnouncementFormData) => {
    ...
    if (formData.attachmentFile) {
      body.append('attachmentFile', formData.attachmentFile);
    }

    try {
      const response = await fetch(endpoint, { method, body });
      if (!response.ok) throw new Error('Failed to save announcement');
      setNotification({ message: `Announcement ${selectedAnnouncement ? 'updated' : 'created'} successfully!`, type: 'success' });
      fetchAnnouncements();
    } catch (error) {
      console.error('Error saving announcement:', error);
      setNotification({ message: 'Failed to save announcement.', type: 'error' });
    }
        ...
        filteredAnnouncements.length > 0 ? (
          <div className="divide-y divide-gray-200">
            {filteredAnnouncements.map((ann) => (
              <div key={ann.announcement_id} className="p-4 flex flex-col sm:flex-row justify-between sm:items-center">
                <div className="flex-1 mb-4 sm:mb-0">
                    <p className="text-sm text-gray-500">{new Date(ann.publish_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                ...
                <div className="flex-shrink-0 flex space-x-2 self-end sm:self-center">
                  ...
                  <button onClick={() => handleDeleteClick(ann.announcement_id)} className="flex items-center justify-center px-3 py-2 text-sm font-medium text-white bg-red-600 rounded-lg shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors" title="Delete Announcement">
...
function ViewAnnouncementModal({ isOpen, onClose, announcement }: { isOpen: boolean; onClose: () => void; announcement: Announcement | null; }) {
  ...
  const attachmentUrl = announcement.attachment;
    ...
    <div className="fixed inset-0 backdrop-blur-sm flex justify-center items-center z-50 p-4 transition-opacity">
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-5xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 flex-grow overflow-y-auto">
          <p className="text-sm text-gray-500 mb-2">{new Date(announcement.publish_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
          ...
          {attachmentUrl && (
            <div className="border-t border-gray-200 pt-4">
              {isImage(attachmentUrl) ? (
                <Image src={attachmentUrl} alt="Attachment" width={800} height={600} className="max-w-full h-auto rounded-md border" />
              ) : isPdf(attachmentUrl) ? (
                <embed src={attachmentUrl} type="application/pdf" className="w-full h-[50vh] border rounded-md" />
              ) : (
                <div className="p-4 bg-gray-100 rounded-md">
                  <p className="text-gray-700">Preview is not available for this file type.</p>
                  <a href={attachmentUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline font-semibold">
                    Download Attachment
                  </a>
                </div>
              )}
...
function AnnouncementModal({ isOpen, onClose, onSave, announcement }: { isOpen: boolean; onClose: () => void; onSave: (data: AnnouncementFormData) => void; announcement: Announcement | null; }) {
  const [formData, setFormData] = useState<AnnouncementFormData>({ title: '', content: '', publish_date: '', attachmentFile: null });
  
  useEffect(() => {
    if (announcement) {
      setFormData({ title: announcement.title, content: announcement.content, publish_date: announcement.publish_date, attachmentFile: null });
    } else {
      setFormData({ title: '', content: '', publish_date: new Date().toISOString().split('T')[0], attachmentFile: null });
    }
  }, [announcement, isOpen]);
    ...
    <div className="fixed inset-0 backdrop-blur-sm flex justify-center items-center z-50 p-4 transition-opacity">
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-2xl w-full max-w-3xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 flex-grow overflow-y-auto space-y-4">
          ...
          <div><label htmlFor="publish_date" className="block text-sm font-medium text-gray-700">Publish Date</label><input type="date" name="publish_date" id="publish_date" value={formData.publish_date} onChange={handleChange} required className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500"/></div>
          <div>
            <label htmlFor="attachmentFile" className="block text-sm font-medium text-gray-700 mb-1">Attachment (Optional)</label>
            <input type="file" name="attachmentFile" id="attachmentFile" onChange={handleChange} className="block w-full text-sm text-gray-700 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 border border-gray-300 rounded-md shadow-sm p-0 focus:ring-indigo-500 focus:border-indigo-500"/>
            <div className="mt-2 text-sm text-gray-500">
              {formData.attachmentFile ? `Selected: ${formData.attachmentFile.name}` : 'No file chosen'}
            </div>
          </div>
        </div>
