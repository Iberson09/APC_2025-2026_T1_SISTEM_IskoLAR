...
type User = { 
  ...
  course?: string | null;
  role_id: string;
  role: UserRole; // This comes from the API join
  status: UserStatus; // You may need to add this to your schema or derive it
  created_at: string;
  updated_at: string;
  last_login?: string | null; // You may need to add this field to track login activity
  activityLog?: ActivityLog[]; // Optional for now since it's not in DB
};
...
const UserPill = ({ type, value }: { type: 'role' | 'status', value: UserRole | UserStatus | string }) => {
  const configs = {
    role: {
      ...
      User: { bg: 'bg-purple-50', text: 'text-purple-700', border: 'border-purple-200' },
    },
    status: {
  ...
  // FIX: Added a fallback for unknown values
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium ${config.bg} ${config.text} border ${config.border}`}>
      <span className={`h-1.5 w-1.5 rounded-full flex-shrink-0 ${getDotColor()}`}></span>
      {value || 'N/A'}
    </span>
  );
};
...
const formatLastLogin = (dateString?: string | null) => {
  ...
  let color = '';
  if (diffInMonths < 2) { 
    if (diffInDays <= 0) text = 'Today'; 
    else if (diffInDays === 1) text = 'Yesterday'; 
    else text = `${diffInDays} days ago`; 
    color = 'text-green-600'; 
  }
  else if (diffInMonths < 3) { 
    text = 'Over 2 months ago'; 
    color = 'text-green-600'; 
  }
  else { 
...
export default function UserManagementPage() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      // Transform the data to include status (you may need to derive this from your schema)
      const transformedData = data.map((user: User) => ({
        ...user,
        status: 'Pending' as UserStatus, // Default status - you may want to add this field to your schema
        activityLog: [] // Empty for now since it's not in your schema
      }));
    ...
    return users
      .filter(user => {
        ...
        if(filters.lastLogin !== 'all' && user.last_login) {
            ...
            matchesLastLogin = filters.lastLogin === '3months'; // Treat 'never' as 'over 3 months'
        }
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredUsers.map(user => (
                  <tr key={user.user_id} className={`group transition-colors duration-150 ${selectedUserIds.includes(user.user_id) ? 'bg-blue-50' : 'hover:bg-gray-50'}`}>
                    <td className="p-4 text-center"><input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" checked={selectedUserIds.includes(user.user_id)} onChange={() => handleSelectUser(user.user_id)} /></td>
...
function UserFilterModal({ isOpen, onClose, onApply, onReset, initialFilters, counts }: UserFilterModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/25 overflow-y-auto h-full w-full z-50">
      <div className="relative top-20 mx-auto p-5 w-full max-w-md bg-white rounded-lg shadow-xl">
...
// Updated UserDetailsModal to show new fields
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 flex-grow overflow-y-auto space-y-6">
          ...
          {(user.present_address || user.permanent_address) && (
            <div>
              <h3 className="font-semibold text-gray-800 mb-2">Address Information</h3>
              <div className="text-sm space-y-1">
                {user.present_address && <p><strong>Present Address:</strong> {user.present_address}</p>}
          ...
          {(user.college_university || user.course) && (
            <div>
              ...
              <div className="text-sm space-y-1">
                {user.college_university && <p><strong>College/University:</strong> {user.college_university}</p>}
          ...
          {user.activityLog && user.activityLog.length > 0 && (
            <div>
              <h3 className="font-semibold text-gray-800 mb-2">Recent Activity Log</h3>
              <ul className="text-sm space-y-2 border rounded-md p-2 max-h-48 overflow-y-auto">
                {user.activityLog.map(log => (
                  <li key={log.id} className="border-b last:border-b-0 pb-1">
                    <p><strong>Action:</strong> {log.action} - <span className="text-gray-500">{new Date(log.timestamp).toLocaleString()}</span></p>
